...
class MemberManager(QWidget):
    ...
    def del_setting_box(self):
        ...
        if choice.exec():
            ...
            member_dict = Config.search_member()
            move_dict = {
                k: v for k, v in member_dict.items() if int(k[3:]) > int(name[3:])
            }

            self.member_manager.clear_SettingBox()

            shutil.rmtree(member_dict[name]["Path"])
            Config.change_queue(name, "禁用")
            for index, info in move_dict.items():
                if info["Path"].exists():
                    info["Path"].rename(
                        info["Path"].with_name(f"脚本_{int(index[3:])-1}")
                    )
                Config.change_queue(index, f"脚本_{int(index[3:])-1}")
    ...
    def left_setting_box(self):
        ...
        member_dict[name]["Path"].rename(member_dict[name]["Path"].with_name("脚本_0"))
        Config.change_queue(name, "脚本_0")
        member_dict[f"脚本_{index-1}"]["Path"].rename(member_dict[name]["Path"])
        Config.change_queue(f"脚本_{index-1}", name)
        member_dict[name]["Path"].with_name("脚本_0").rename(
            member_dict[f"脚本_{index-1}"]["Path"]
        )
    ...
    class MemberSettingBox(QWidget):
        ...
        def show_SettingBox(self, index) -> None:
            ...
            for name, info in member_dict.items():
                if info["Type"] == "Maa":
                    Config.open_database(info["Type"], name)
        ...
        def switch_SettingBox(self, index: int, if_chang_pivot: bool = True) -> None:
            ...
            qconfig.load(
                member_dict[f"脚本_{index}"]["Path"] / "config.json",
                Config.maa_config,
            )
            Config.open_database(member_dict[f"脚本_{index}"]["Type"], f"脚本_{index}")
            self.script_list[index - 1].user_setting.user_list.update_user_info(
                "normal"
            )
        ...
        class MaaSettingBox(QWidget):
            ...
            class AppSettingCard(HeaderCardWidget):
                ...
                def PathClicked(self):
                    ...
                    if (
                        not folder
                        or Config.maa_config.get(Config.maa_config.MaaSet_Path)
                        == folder
                    ):
                    ...
                    shutil.copy(
                        ...
                        Config.app_path
                        / f"config/MaaConfig/{self.name}/Default/gui.json",
                    )
            ...
            class UserSettingCard(HeaderCardWidget):
                ...
                def __init__(self, parent=None, name: str = None):
                    ...
                    self.tools.addActions(
                        [
                            Action(
                                FluentIcon.ADD,
                                "新建用户",
                                triggered=self.user_list.add_user,
                            ),
                            Action(
                                FluentIcon.REMOVE,
                                "删除用户",
                                triggered=self.user_list.del_user,
                            ),
                        ]
                    )
                    self.tools.addSeparator()
                    self.tools.addActions(
                        [
                            Action(
                                FluentIcon.UP,
                                "向上移动",
                                triggered=self.user_list.up_user,
                            ),
                            Action(
                                FluentIcon.DOWN,
                                "向下移动",
                                triggered=self.user_list.down_user,
                            ),
                        ]
                    )
                    self.tools.addSeparator()
                    self.tools.addAction(
                        Action(
                            FluentIcon.SCROLL,
                            "模式转换",
                            triggered=self.user_list.switch_user,
                        )
                    )
                    self.tools.addSeparator()
                    self.tools.addAction(
                ...
                def set_more(self):
                    ...
                    if (
                        self.user_list.pivot.currentRouteKey()
                        == f"{self.name}_简洁用户列表"
                    ):
                ...
                class UserListBox(QWidget):
                    ...
                    def update_user_info(self, operation: str) -> None:
                        ...
                        if self.if_user_list_editable:
                            self.user_list_simple.setEditTriggers(
                                TableWidget.AllEditTriggers
                            )
                            self.user_list_beta.setEditTriggers(
                                TableWidget.AllEditTriggers
                            )
                        else:
                            self.user_list_simple.setEditTriggers(
                                TableWidget.NoEditTriggers
                            )
                            self.user_list_beta.setEditTriggers(
                                TableWidget.NoEditTriggers
                            )
