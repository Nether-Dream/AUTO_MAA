...
class QueueManager(QWidget):
    ...
    def add_setting_box(self):
        ...
        index = len(Config.search_queue()) + 1

        qconfig.load(
            Config.app_path / f"config/QueueConfig/调度队列_{index}.json",
            Config.queue_config,
        )
        Config.queue_config.clear()
    ...
    def del_setting_box(self):
        ...
        if choice.exec():
            ...
            move_dict = {
                k: v for k, v in queue_dict.items() if int(k[5:]) > int(name[5:])
            }

            self.queue_manager.clear_SettingBox()

            queue_dict[name]["Path"].unlink()
            for index, info in move_dict.items():
                if info["Path"].exists():
                    info["Path"].rename(
                        info["Path"].with_name(f"调度队列_{int(index[5:])-1}.json")
                    )
    ...
    def left_setting_box(self):
        ...
        queue_dict[name]["Path"].rename(
            queue_dict[name]["Path"].with_name("调度队列_0.json")
        )
        queue_dict[f"调度队列_{index-1}"]["Path"].rename(queue_dict[name]["Path"])
        queue_dict[name]["Path"].with_name("调度队列_0.json").rename(
            queue_dict[f"调度队列_{index-1}"]["Path"]
        )
        ...
        class QueueMemberSettingBox(QWidget):
            ...
            class TaskSettingCard(HeaderCardWidget):
                ...
                def __init__(self, parent=None):
                    ...
                    member_list = [
                        ["禁用"] + [_ for _ in member_dict.keys()],
                        ["未启用"]
                        + [
                            (
                                k
                                if v["Data"]["MaaSet"]["Name"] == ""
                                else f"{k} - {v["Data"]["MaaSet"]["Name"]}"
                            )
